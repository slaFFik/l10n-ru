...
include(dirname(__FILE__).'/rss_parser.php');
...
class smi2 {
	...
	function ask () {
		echo '
			...
			<div class="inside">
				...
				<br />
				Используя кросс-постинг блоггер обязуется показывать блок ссылок по теме. В противном случае администрация сайта СМИ2 может отключить возможность кросспостинга для данного пользователя.
			</div>
			</div>
		';
	}
	...
	function publish ($ID) {
		...
		$tagline = '';
		if (count(get_the_tags($ID))) foreach (get_the_tags($ID) as $tag) $tagline .= ','.$tag->name;
		$tagline = substr($tagline,1);
		$count = $this->count;
		
		$rss->file = "http://smi2.ru/export/rss/actual.php?tags=$tagline&count=$count";
		...
		foreach ($rss->channel['ITEM'] as $item) {
			$smi2_str .= "<li><a href='".$item["GUID"]."?master=".urlencode($this->name)."'>".$item["TITLE"]."</a></li>";
		}
		$smi2_str .= "</ul>";
