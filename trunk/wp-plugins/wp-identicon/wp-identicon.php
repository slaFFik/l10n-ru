<?php
/*
Plugin Name: WordPress Identicon
Version: 1.02
...
function identicon_textdomain() {
	load_plugin_textdomain('wp-identicon', 'wp-content/plugins/wp-identicon/i18n');
}
add_action('init', 'identicon_textdomain');

function identicon_menu() {
	if (function_exists('add_options_page')) {
		add_options_page(__('Identicon Control Panel', 'wp-identicon'), __('Identicon Options', 'wp-identicon'), 1, basename(__FILE__), 'identicon_subpanel');
	}
}
...
class identicon {
	...
	function identicon_build($seed='',$altImgText='',$img=true,$outsize='',$write=true,$random=true,$displaysize='',$gravataron=true){
        ...
        $filename = "http://www.gravatar.com/avatar.php?gravatar_id=".md5($seed)."&amp;size=$outsize&amp;default=$filename";
			if ($img){
				$filename='<img class="identicon" src="'.$filename.'" alt="'.str_replace('"',"'",$altImgText).__(' Identicon Icon', 'wp-identicon').'" height="'.$displaysize.'" width="'.$displaysize.'" />';
			}
			return $filename;
...
function identicon_subpanel() {
	...
	?>
<div class="wrap"> 
<h2><?php _e('Identicon Options.', 'wp-identicon'); ?></h2>
	
<p><?php _e('You currently have', 'wp-identicon'); ?> <?php echo $identicon_count;?> <?php _e('identicons on your website', 'wp-identicon'); ?>.</p>

<p><?php _e('Set options here:', 'wp-identicon'); ?></p>

<form method="post" action="options-general.php?page=wp-identicon.php">
<ul style="list-style-type: none">
<li><strong><?php _e('Identicon Size (in pixels)', 'wp-identicon'); ?></strong>:<br /> 
<input type="text" name="identiconsize" size="5" value="<?php echo $identicon_options['size'];?>"/>
<small><?php _e('(Default: 35)', 'wp-identicon'); ?></small></li>
<li><strong><?php _e('Number of Squares', 'wp-identicon'); ?></strong>:<br /> 
<input type="text" name="identiconsquares" size="5" value="<?php echo $identicon_options['squares'];?>"/>
<small><?php _e('(Default: 4)', 'wp-identicon'); ?></small><br/><br/></li>
...
<li><strong><?php _e('Background Colors', 'wp-identicon'); ?></strong>:<br />
	<small><?php _e('(enter single value or range. 0,0,0 for transparent. Default: 255,255,255)', 'wp-identicon'); ?></small><br/>
	<?php echo $input_detail['r'];?><input type="text" name="backr" size="10" <?php echo $input_back['r'];?> value="<?php echo implode($identicon_options['backr'],'-');?>"/>
	<?php echo $input_detail['g'];?><input type="<?php echo $gbtype;?>" name="backg" size="10" <?php echo $input_back['g'];?> value="<?php echo implode($identicon_options['backg'],'-');?>"/>
	<?php echo $input_detail['b'];?><input type="<?php echo $gbtype;?>" name="backb" size="10" <?php echo $input_back['b'];?> value="<?php echo implode($identicon_options['backb'],'-');?>"/><br/><br/></li>
	<li><strong><?php _e('Foreground Colors', 'wp-identicon'); ?>:</strong><br/>
	<small><?php _e('(enter single value or range. Default: 1-255,1-255,1-255)', 'wp-identicon'); ?></small><br/>
	<?php echo $input_detail['r'];?><input type="text" name="forer" size="10" <?php echo $input_back['r'];?> value="<?php echo implode($identicon_options['forer'],'-');?>"/>
	<?php echo $input_detail['g'];?><input type="<?php echo $gbtype;?>" name="foreg" size="10" <?php echo $input_back['g'];?> value="<?php echo implode($identicon_options['foreg'],'-');?>"/>
	<?php echo $input_detail['b'];?><input type="<?php echo $gbtype;?>" name="foreb" size="10" <?php echo $input_back['b'];?> value="<?php echo implode($identicon_options['foreb'],'-');?>"/><br/><br/></li>
	<li><strong><?php _e('Grayscale', 'wp-identicon'); ?>:</strong> <input type="checkbox" name="grey" value="1" <?php if ($identicon_options['grey']) echo 'checked="checked"';?> /> 
	<small><?php _e('(Good for black and white themes)', 'wp-identicon'); ?></small><br/><br/></li>
	
	<li><strong><?php _e('Automatically Add Identicons to Comments', 'wp-identicon'); ?></strong><br/>
	<small><?php _e('(adds an Identicon beside commenter names or disable it and edit theme file manually). Default: Auto)', 'wp-identicon'); ?></small><br /> <input type="radio" name="autoadd" value="0" <?php if (!$identicon_options['autoadd']) echo 'checked="checked"';?>/> <?php _e("I'll Do It Myself", 'wp-identicon'); ?>&nbsp;&nbsp;&nbsp;<input type="radio" name="autoadd" value="1" <?php if ($identicon_options['autoadd']==1) echo 'checked="checked"';?>/> <?php _e('Add Identicons For Me', 'wp-identicon'); ?>&nbsp;&nbsp;&nbsp;<input type="radio" name="autoadd" value="2" <?php if ($identicon_options['autoadd']==2) echo 'checked="checked"';?>/> <?php _e('My Theme Has Builtin WordPress 2.5+ Avatars', 'wp-identicon'); ?></li>
	
	<li><strong><?php _e('Gravatar Support', 'wp-identicon'); ?>:</strong><br />
	<small><?php _e('(If a commenter has a gravatar use it, otherwise use Identicon. Default: Identicon Only)', 'wp-identicon'); ?></small><br/>
	<input type="radio" name="gravatar" value="0" <?php if (!$identicon_options['gravatar']) echo 'checked="checked"';?>/> <?php _e('Identicon Only', 'wp-identicon'); ?>&nbsp;&nbsp;&nbsp;<input type="radio" name="gravatar" value="1" <?php if ($identicon_options['gravatar']) echo 'checked="checked"';?>/> <?php _e('Gravatar + Identicon', 'wp-identicon'); ?></li>
	<li><input type="submit" name="submit" value="<?php _e('Set Options', 'wp-identicon'); ?>"/></li>
	</ul>
	</form>

<h4><?php _e('Clear the Identicon Image Cache', 'wp-identicon'); ?>:</h4> 

<form method="post" action="options-general.php?page=wp-identicon.php">
<input type="submit" name="clear" value="<?php _e('Clear Cache', 'wp-identicon'); ?>"/></form>
...
<?php if (!is_writable(''.WP_IDENTICON_DIR_INTERNAL)){echo '<div class="error"><p>'.__('Identicon needs ', 'wp-identicon').WP_IDENTICON_DIR_INTERNAL.__(' to be writable.', 'wp-identicon').'</p></div>';}
	if (!function_exists("gd_info")){echo '<div class="error"><p>'.__('GD Image library not found. Identicon needs this library.', 'wp-identicon').'</p></div>';}?>
<p><?php _e('A test identicon should be here', 'wp-identicon'); ?>:<br/><br/><?php $identicon=new identicon; echo $identicon->identicon_build('This is a test','Test');?><br/><br/><?php _e('and the source URL for this image is', 'wp-identicon'); ?> <a href="<?php echo $identicon->identicon_build('This is a test','Test',false);?>"><?php _e('here', 'wp-identicon'); ?></a>.</p>
<p><?php _e('If there is no identicon above or there are any other problems, concerns or suggestions please let me know <a href="http://scott.sherrillmix.com/blog/blogger/wp-identicon">here</a>. Enjoy your identicons.', 'wp-identicon'); ?></p>

<p><?php _e("For curiosity's sake, here are the parts the identicons are built from", 'wp-identicon'); ?>:</p>
