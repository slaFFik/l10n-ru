...
add_action('init', 'recaptcha_textdomain');
function recaptcha_textdomain() {
	load_plugin_textdomain('wp-recaptcha', false, 'wp-recaptcha');
}
...
$option_defaults = array (
   ...
   'error_blank' => __('<strong>ERROR</strong>: Please fill in the reCAPTCHA form.', "wp-recaptcha"), // the message to display when the user enters no CAPTCHA response
   'error_incorrect' => __('<strong>ERROR</strong>: That reCAPTCHA response was incorrect.', "wp-recaptcha"), // the message to display when the user enters the incorrect CAPTCHA response
);

// install the defaults
if ($wpmu != 1)
   add_option('recaptcha', $option_defaults, __('reCAPTCHA Default Options', "wp-recaptcha"), 'yes');
...
function recaptcha_wp_options_subpanel() {
   global $wpmu;
	...
	$optionarray_def = array(
      ...
      'mh_replace_link' => '',
      'mh_replace_title' => '',
      'error_blank' => __('<strong>ERROR</strong>: Please fill in the reCAPTCHA form.', "wp-recaptcha"),
      'error_incorrect' => __('<strong>ERROR</strong>: That reCAPTCHA response was incorrect.', "wp-recaptcha"),
		);
...
function recaptcha_dropdown_capabilities($select_name, $checked_value="") {
	...
	$capability_choices = array (
	 	__('All registered users', "wp-recaptcha") => 'read',
	 	__('Edit posts', "wp-recaptcha") => 'edit_posts',
	 	__('Publish Posts', "wp-recaptcha") => 'publish_posts',
	 	__('Moderate Comments', "wp-recaptcha") => 'moderate_comments',
	 	__('Administer site', "wp-recaptcha") => 'level_10'
	 	);
...
<!-- ############################## BEGIN: ADMIN OPTIONS ################### -->
<div class="wrap">
	<h2><?php _e('reCAPTCHA Options', 'wp-recaptcha'); ?></h2>
	<h3><?php _e('About reCAPTCHA', 'wp-recaptcha'); ?></h3>
	<p><?php _e('reCAPTCHA is a free, accessible CAPTCHA service that helps to digitize books while blocking spam on your blog.', 'wp-recaptcha'); ?></p>
	
	<p><?php _e('reCAPTCHA asks commenters to retype two words scanned from a book to prove that they are a human. This verifies that they are not a spambot while also correcting the automatic scans of old books. So you get less spam, and the world gets accurately digitized books. Everybody wins! For details, visit the <a href="http://recaptcha.net/">reCAPTCHA website</a>.', 'wp-recaptcha'); ?></p>
   <p><?php _e('<strong>NOTE</strong>: If you are using some form of Cache plugin you will probably need to flush/clear your cache for changes to take effect.', 'wp-recaptcha'); ?></p>
   
	<form name="form1" method="post" action="<?php echo $_SERVER['REDIRECT_SCRIPT_URI'] . '?page=' . plugin_basename(__FILE__); ?>&updated=true">
		<div class="submit">
			<input type="submit" name="submit" value="<?php _e('Update Options', 'wp-recaptcha') ?> &raquo;" />
		</div>
	
	<!-- ****************** Operands ****************** -->
   <table class="form-table">
   <tr valign="top">
		<th scope="row"><?php _e('reCAPTCHA Keys', 'wp-recaptcha'); ?></th>
		<td>
			<?php _e('reCAPTCHA requires an API key, consisting of a "public" and a "private" key.', 'wp-recaptcha'); ?> <?php _e('You can sign up for a', 'wp-recaptcha'); ?> <a href="<?php echo recaptcha_get_signup_url (recaptcha_wp_blog_domain (), 'wordpress');?>" target="0"><?php _e('free reCAPTCHA key', 'wp-recaptcha'); ?></a>.
			<br />
			<p class="re-keys">
				<!-- reCAPTCHA public key -->
				<label class="which-key" for="recaptcha_opt_pubkey"><?php _e('Public Key:', 'wp-recaptcha'); ?></label>
				<input name="recaptcha_opt_pubkey" id="recaptcha_opt_pubkey" size="40" value="<?php  echo $optionarray_def['pubkey']; ?>" />
				<br />
				<!-- reCAPTCHA private key -->
				<label class="which-key" for="recaptcha_opt_privkey"><?php _e('Private Key:', 'wp-recaptcha'); ?></label>
				<input name="recaptcha_opt_privkey" id="recaptcha_opt_privkey" size="40" value="<?php  echo $optionarray_def['privkey']; ?>" />
			</p>
	    </td>
    </tr>
  	<tr valign="top">
		<th scope="row"><?php _e('Comment Options', 'wp-recaptcha'); ?></th>
		<td>
			<!-- Show reCAPTCHA on the comment post -->
			<big><input type="checkbox" name="re_comments" id="re_comments" value="1" <?php if($optionarray_def['re_comments'] == true){echo 'checked="checked"';} ?> /> <label for="re_comments"><?php _e('Enable reCAPTCHA for comments.', 'wp-recaptcha'); ?></label></big>
			<br />
			...
			<label name="re_bypass" for="re_bypass"><?php _e('Hide reCAPTCHA for <strong>registered</strong> users who can:', 'wp-recaptcha'); ?></label>
			<?php recaptcha_dropdown_capabilities('re_bypasslevel', $optionarray_def['re_bypasslevel']); // <select> of capabilities ?>
			</div>

			<!-- The theme selection -->
			<div class="theme-select">
			<label for="re_theme"><?php _e('Theme:', 'wp-recaptcha'); ?></label>
			<select name="re_theme" id="re_theme">
			<option value="red" <?php if($optionarray_def['re_theme'] == 'red'){echo 'selected="selected"';} ?>><?php _e('Red', 'wp-recaptcha'); ?></option>
			<option value="white" <?php if($optionarray_def['re_theme'] == 'white'){echo 'selected="selected"';} ?>><?php _e('White', 'wp-recaptcha'); ?></option>
			<option value="blackglass" <?php if($optionarray_def['re_theme'] == 'blackglass'){echo 'selected="selected"';} ?>><?php _e('Black Glass', 'wp-recaptcha'); ?></option>
			<option value="clean" <?php if($optionarray_def['re_theme'] == 'clean'){echo 'selected="selected"';} ?>><?php _e('Clean', 'wp-recaptcha'); ?></option>
			</select>
			</div>
			<!-- Tab Index -->
			<label for="re_tabindex"><?php _e('Tab Index (<em>e.g. WP: <strong>5</strong>, WPMU: <strong>3</strong></em>):', 'wp-recaptcha'); ?></label>
	...
	<tr valign="top">
		<th scope="row"><?php _e('Registration Options', 'wp-recaptcha'); ?></th>
		<td>
			<!-- Show reCAPTCHA on the registration page -->
			<big><input type="checkbox" name="re_registration" id="re_registration" value="1" <?php if($optionarray_def['re_registration'] == true){echo 'checked="checked"';} ?> /> <label for="re_registration"><?php _e('Enable reCAPTCHA on registration form.', 'wp-recaptcha'); ?></label></big>
			<br />
			<!-- The theme selection -->
			<div class="theme-select">
				<label for="re_theme_reg"><?php _e('Theme:', 'wp-recaptcha'); ?></label>
				<select name="re_theme_reg" id="re_theme_reg">
				<option value="red" <?php if($optionarray_def['re_theme_reg'] == 'red'){echo 'selected="selected"';} ?>><?php _e('Red', 'wp-recaptcha'); ?></option>
				<option value="white" <?php if($optionarray_def['re_theme_reg'] == 'white'){echo 'selected="selected"';} ?>><?php _e('White', 'wp-recaptcha'); ?></option>
				<option value="blackglass" <?php if($optionarray_def['re_theme_reg'] == 'blackglass'){echo 'selected="selected"';} ?>><?php _e('Black Glass', 'wp-recaptcha'); ?></option>
				<option value="clean" <?php if($optionarray_def['re_theme_reg'] == 'clean'){echo 'selected="selected"';} ?>><?php _e('Clean', 'wp-recaptcha'); ?></option>
				</select>
			</div>
			<?php } ?>
		</td>
   ...
   <tr valign="top">
      <th scope="row"><?php _e('Error Messages', 'wp-recaptcha'); ?></th>
         <td>
            <p><?php _e('The following are the messages to display when the user does not enter a CAPTCHA response or enters the incorrect CAPTCHA response.', 'wp-recaptcha'); ?></p>
            <!-- Error Messages -->
            <p class="re-keys">
               <!-- Blank -->
      			<label class="which-key" for="error_blank"><?php _e('No response entered:', 'wp-recaptcha'); ?></label>
      			<input name="error_blank" id="error_blank" size="80" value="<?php echo $optionarray_def['error_blank']; ?>" />
      			<br />
      			<!-- Incorrect -->
      			<label class="which-key" for="error_incorrect"><?php _e('Incorrect response entered:', 'wp-recaptcha'); ?></label>
	 ...
	 <tr valign="top">
			<th scope="row"><?php _e('General Settings', 'wp-recaptcha'); ?></th>
			<td>
				<!-- The language selection -->
				<div class="lang-select">
				<label for="re_lang"><?php _e('Language:', 'wp-recaptcha'); ?></label>
				<select name="re_lang" id="re_lang">
				<option value="en" <?php if($optionarray_def['re_lang'] == 'en'){echo 'selected="selected"';} ?>><?php _e('English', 'wp-recaptcha'); ?></option>
				<option value="nl" <?php if($optionarray_def['re_lang'] == 'nl'){echo 'selected="selected"';} ?>><?php _e('Dutch', 'wp-recaptcha'); ?></option>
				<option value="fr" <?php if($optionarray_def['re_lang'] == 'fr'){echo 'selected="selected"';} ?>><?php _e('French', 'wp-recaptcha'); ?></option>
				<option value="de" <?php if($optionarray_def['re_lang'] == 'de'){echo 'selected="selected"';} ?>><?php _e('German', 'wp-recaptcha'); ?></option>
				<option value="pt" <?php if($optionarray_def['re_lang'] == 'pt'){echo 'selected="selected"';} ?>><?php _e('Portuguese', 'wp-recaptcha'); ?></option>
				<option value="ru" <?php if($optionarray_def['re_lang'] == 'ru'){echo 'selected="selected"';} ?>><?php _e('Russian', 'wp-recaptcha'); ?></option>
				<option value="es" <?php if($optionarray_def['re_lang'] == 'es'){echo 'selected="selected"';} ?>><?php _e('Spanish', 'wp-recaptcha'); ?></option>
				<option value="tr" <?php if($optionarray_def['re_lang'] == 'tr'){echo 'selected="selected"';} ?>><?php _e('Turkish', 'wp-recaptcha'); ?></option>
				</select>
				</label>
		    	</div>
		    	<!-- Whether or not to be XHTML 1.0 Strict compliant -->
				<input type="checkbox" name="re_xhtml" id="re_xhtml" value="1" <?php if($optionarray_def['re_xhtml'] == true){echo 'checked="checked"';} ?> /> <label for="re_xhtml"><?php _e("Be XHTML 1.0 Strict compliant. <strong>Note</strong>: Bad for users who don't have Javascript enabled in their browser (Majority do).", "wp-recaptcha"); ?></label>
				<br />
			</td>
		</tr>
	</table>
	
	<h3><?php _e('About MailHide', 'wp-recaptcha'); ?></h3>
	<p><a href="http://mailhide.recaptcha.net/" title="mailhide email obfuscation">MailHide</a> <?php _e('uses reCAPTCHA to protect email adresses displayed on your blog from being harvested for spam.', 'wp-recaptcha'); ?></p>
	<p><?php _e('Activating MailHide will automatically hide all emails in posts and comments from spam bots. For example, support@recaptcha.net would become', 'wp-recaptcha'); ?> supp<a href="http://mailhide.recaptcha.net/d?k=01a8k2oW96qNZ4JhiFx5zDRg==&amp;c=yifPREOOvfzA0o3dbnnwP8fy91UD8RL4SspHDIKHVRE=" onclick="window.open('http://mailhide.recaptcha.net/d?k=01a8k2oW96qNZ4JhiFx5zDRg==&amp;c=yifPREOOvfzA0o3dbnnwP8fy91UD8RL4SspHDIKHVRE=', '', 'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=500,height=300'); return false;" title="<?php _e('Reveal this e-mail address', 'wp-recaptcha'); ?>">...</a>@recaptcha.net. <?php _e('There are also options below for choosing the text to show for hidden emails.', 'wp-recaptcha'); ?></p>
	<p><?php _e('MailHide also requires a public and private key which you can generate using the <a href="http://mailhide.recaptcha.net/apikey">key generation service</a>.', 'wp-recaptcha'); ?></p>
	<table class="form-table">
	<tr valign="top">
	<th scope="row"><?php _e('MailHide Keys', 'wp-recaptcha'); ?></th>
	<td>
		<!-- MailHide Enabler -->
		<big><?php _e('Enable MailHide email obfuscation for:', 'wp-recaptcha'); ?></big><br />
		   <input type="checkbox" name="use_mailhide_posts" id="use_mailhide_posts" value="1" <?php if($optionarray_def['use_mailhide_posts'] == true){echo 'checked="checked"';} ?> /><label for="use_mailhide_posts"><?php _e('Posts/Pages', 'wp-recaptcha'); ?></label><br />
		   <input type="checkbox" name="use_mailhide_comments" id="use_mailhide_comments" value="1" <?php if($optionarray_def['use_mailhide_comments'] == true){echo 'checked="checked"';} ?> /><label for="use_mailhide_comments"><?php _e('Comments', 'wp-recaptcha'); ?></label><br />
		   <input type="checkbox" name="use_mailhide_rss" id="use_mailhide_rss" value="1" <?php if($optionarray_def['use_mailhide_rss'] == true){echo 'checked="checked"';} ?> /><label for="use_mailhide_rss"><?php _e('RSS Feed of Posts/Pages', 'wp-recaptcha'); ?></label><br />
		   <input type="checkbox" name="use_mailhide_rss_comments" id="use_mailhide_rss_comments" value="1" <?php if($optionarray_def['use_mailhide_rss_comments'] == true){echo 'checked="checked"';} ?> /><label for="use_mailhide_rss_comments"><?php _e('RSS Feed of Comments', 'wp-recaptcha'); ?></label><br />
		<!-- Public Key -->
		<p class="re-keys">
			<label class="which-key" for="mailhide_pub"><?php _e('Public Key:', 'wp-recaptcha'); ?></label>
			<input name="mailhide_pub" id="mailhide_pub" size="40" value="<?php echo $optionarray_def['mailhide_pub']; ?>" />
			<br />
			<!-- Private Key -->
			<label class="which-key" for="mailhide_priv"><?php _e('Private Key:', 'wp-recaptcha'); ?></label>
			<input name="mailhide_priv" id="mailhide_priv" size="40" value="<?php echo $optionarray_def['mailhide_priv']; ?>" />
		</p>
   </td>
   </tr>
   <tr valign="top">
   <th scope="row"><?php _e('Visibility Options', 'wp-recaptcha'); ?></th>
   <td>
		...
		<div class="theme-select">
			...
			<label for="mh_bypass"><?php _e('Show full email adresses to <strong>registered</strong> users who can:', 'wp-recaptcha'); ?></label>
...
if ( !($recaptcha_opt ['pubkey'] && $recaptcha_opt['privkey'] ) && !isset($_POST['submit']) ) {
   function recaptcha_warning() {
		...
		$top = 7;
		echo "
		<div id='recaptcha-warning' class='updated fade-ff0000'><p><strong>".__("reCAPTCHA is not active", "wp-recaptcha")."</strong> ".__("You must", "wp-recaptcha")." <a href='options-general.php?page=" . $path . "'>".__("enter your reCAPTCHA API key", "wp-recaptcha")."</a> ".__("for it to work", "wp-recaptcha")."</p></div>
		<style type='text/css'>
...
if (($mailhide_enabled && !extension_loaded('mcrypt')) && !isset($_POST['submit'])) {
   function mcrypt_warning() {
		...
		$top = 7;
		echo "
		<div id='recaptcha-warning' class='updated fade-ff0000'><p><strong>".__("MailHide is not active", "wp-recaptcha")."</strong> ".__("You must have the <a href='http://us3.php.net/mcrypt'>mcrypt</a> module loaded for it to work", "wp-recaptcha")."</p></div>
		<style type='text/css'>
		#adminmenu { margin-bottom: 5em; }
		#recaptcha-warning { position: absolute; top: {$top}em; }
		</style>
...
if ($mailhide_enabled && !($recaptcha_opt['mailhide_pub'] && $recaptcha_opt['mailhide_pub']) && !isset($_POST['submit'])) {
	function mailhide_warning() {
		...
		echo "
		<div id='recaptcha-warning' class='updated fade-ff0000'><p><strong>".__("MailHide is not active", "wp-recaptcha")."</strong> ".__("You must", "wp-recaptcha")." <a href='options-general.php?page=" . $path . "'>".__("enter your MailHide API keys", "wp-recaptcha")."</a> ".__("for it to work", "wp-recaptcha")."</p></div>
		<style type='text/css'>
		#adminmenu { margin-bottom: 5em; }
		#recaptcha-warning { position: absolute; top: {$top}em; }
		</style>
