...
<?php if ($cmts) : ?>
	...
	<?php foreach ($cmts[$cmt_ID] as $comment) : ?>
		...
		<li class="<?php echo $comment_background; ?>" id="comment-<?php comment_ID() ?>">
			<div class="authorline"><?php echo sprintf(__('%s on %s at %s', 'yatcp'), get_comment_author_link(), get_comment_date(__('d.m.Y', 'yatcp')), get_comment_time(__('H:i', 'yatcp')));?>
			<?php if (function_exists('edit_comments_xt_link')) { edit_comments_xt_link(); } else { edit_comment_link('edit','&nbsp;&nbsp;',''); } ?>
