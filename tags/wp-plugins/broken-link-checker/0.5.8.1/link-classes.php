	...
	function check(){
        ...
        */
        if ( ($this->check_count >= 3) && ( !$this->timeout ) && ( !$this->http_code ) ) {
        	...
        	$this->log .= "\r\n[".__("A weird error was detected. This should never happen.", "broken-link-checker")."]";
        	$this->save();
            return false;
        }
        ...
        if ( function_exists('curl_init') ) {
            ...
            $this->log .= "=== ".__("First try", "broken-link-checker")." : $code ".(!$code?'(No response) ':'')."===\n\n";
            ...
            if ( (($code<200) || ($code>=400)) && $nobody) {
                ...
                $this->log .= "=== ".__("Second try", "broken-link-checker")." : $code ".(!$code?'(No response) ':'')."===\n\n";
            	$this->log .= $this->last_headers."\n";
            }
        ...
        if ( (($this->http_code>=200) && ($this->http_code<400)) || ($this->http_code == 401) ) {
			...
			if ( $this->http_code == BLC_TIMEOUT ){
				...
				$this->timeout = true;
				$this->log .= "\r\n(".__("Most likely the connection timed out", "broken-link-checker").")";
			}
			return false;
		}
	}
