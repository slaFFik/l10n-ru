...
add_action('init', 'tadv_textdomain');
function tadv_textdomain() {
	if (!function_exists('wp_print_styles')) {
		load_plugin_textdomain('tadv', 'wp-content/plugins/tinymce-advanced/i18n');
	} else {
		load_plugin_textdomain('tadv', false, 'tinymce-advanced/i18n');
	}
}
...
if ( ! function_exists('tadv_activate') ) {
	function tadv_activate() {
		...
		exit(__('<h2>This plugin requires WordPress version 2.6 or newer. Please upgrade your WordPress installation or download an older version of the plugin.</h2>', 'tadv'));
...
if ( ! function_exists('tadv_menu') ) {
	function tadv_menu() {
		if ( function_exists('add_management_page') ) {
			$page = add_management_page( __('TinyMCE Advanced', 'tadv'), __('TinyMCE Advanced', 'tadv'), 9, __FILE__, 'tadv_page' );
