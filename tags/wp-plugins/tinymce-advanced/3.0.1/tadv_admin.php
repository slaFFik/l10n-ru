...
if ( empty($toolbar_1) && empty($toolbar_2) && empty($toolbar_3) && empty($toolbar_4) ) {
    $allbtns = array();
    ?><div class="error" id="message"><p><?php _e('All toolbars are empty!', 'tadv'); ?></p></div><?php
} else {
...
<?php   }
    ...
    <table class="clear" style="margin:10px 0"><tr><td style="padding:2px 12px 8px;">
        ...
        <?php _e('(to show the browser&rsquo;s context menu in Firefox and use the spellchecker, hold down the &quot;Ctrl&quot; key).', 'tadv'); ?> 
        </td></tr>

        <tr><td style="border:1px solid #CD0000;padding:2px 12px 8px;">
        <p style="font-weight:bold;color:#CD0000;"><?php _e('Advanced', 'tadv'); ?></p> 
        
        <p><label for="importcss" class="tadv-box"><?php _e('Import the current theme&rsquo;s CSS classes', 'tadv'); ?>  &nbsp;
        ...
        <p style="font-size:10px;"><?php _e('Custom CSS styles can be added in <code>/wp-content/plugins/tinymce-advanced/css/tadv-mce.css</code>. They will be imported and used in TinyMCE. The file has to be downloaded with FTP, edited and uploaded, overwriting the original. Only CSS classes can be added, also <strong>div.my-class</strong> would not work, but <strong>.my-class</strong> will.', 'tadv'); ?></p>
        ...
        <p style="font-size:10px;"><?php _e('This will make it possible to use more advanced HTML without the back-end filtering affecting it much. It also adds two new buttons to the HTML editor: &quot;autop&quot; that allows wpautop to be run on demand and &quot;undo&quot; that can undo the last changes.', 'tadv'); ?></p>
		</td></tr>
<?php
    ...
    if ( $mce_locale != 'en' ) {
        ...
        <p style="font-weight:bold;"><?php _e('Language Settings'); ?></p>
        <p><?php _e('Your WordPress language is set to ', 'tadv'); ?><strong><?php echo get_locale(); ?></strong>. <?php _e('However there is no matching language installed for TinyMCE&rsquo;s plugins. This plugin includes several translations: German, French, Italian, Spanish, Portuguese, Russian, Japanese and Chinese. More translations are available at <a href="http://services.moxiecode.com/i18n/">TinyMCE&rsquo;s web site</a>.', 'tadv'); ?></p>
		</td></tr>
<?php	}
    } // end mce_locale
?>
...
<p class="submit">
	...
	<input type="submit" name="save" value="<?php _e('Save Changes', 'tadv'); ?>" />
...
<div><?php _e('Uninstalling will remove all saved settings and buttons arrangement from the database.', 'tadv'); ?>
