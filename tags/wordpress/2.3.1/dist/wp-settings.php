...
if ( version_compare( '4.2', phpversion(), '>' ) ) {
	die( 'На Вашем сервере запущен PHP версии ' . phpversion() . ', а для WordPress требуется как минимум 4.2.' );
}

if ( !extension_loaded('mysql') && !file_exists(ABSPATH . 'wp-content/db.php') )
	die( 'Ваш PHP не настроен для работы с MySQL, которая необходима для WordPress.' );
...
if ( preg_match('|[^a-z0-9_]|i', $wpdb->prefix) && !file_exists(ABSPATH . 'wp-content/db.php') )
	wp_die("<strong>ОШИБКА</strong>: <code>$table_prefix</code> в <code>wp-config.php</code> может содержать только цифры, буквы и знаки подчеркивания.");
