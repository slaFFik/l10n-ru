...
<p>Добро пожаловать. Прежде чем мы начнем, потребуется информация о базы данных. Вот что вы должны значть до начала процедуры установки.</p>
<ol>
	<li>Имя базы данных</li>
	<li>Имя пользователя базы данных</li>
	<li>Пароль к базе данных</li>
	<li>Адрес сервера базы данных</li>
	<li>префикс таблиц (если вы хотите запестить более чем один WordPress на одной базе) </li>
</ol>
<p><strong>Если по каким-то причинам это автоматическое создание файла не работает, не расстраивайтесь. Все это предназначено для заполнения настроечного файла. Вы можете просто открыть <code>wp-config-sample.php</code> в текстовом редакторе, внесите вашу информацию и сохраните его как <code>wp-config.php</code>. </strong></p>
...
<?php
	...
	display_header();
?>
<p>Все в порядке! Вы успешно прошли через эту часть установки. WordPress теперь может соединится с вашей базой данных. Если вы готовы, пришло время для &hellip;</p>
