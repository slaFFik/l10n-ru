...
class wpdb {
	...
	function __construct($dbuser, $dbpassword, $dbname, $dbhost) {
		...
		if (!$this->dbh) {
			$this->bail("
<h1>Ошибка установки соединения к базе данных</h1>
<p>Это значит, что имя пользователя и пароль в вашем файле <code>wp-config.php</code> неправильные или мы не может соединится с сервером базы данных <code>$dbhost</code>. Это также может означать, что сервер выключен.</p>
<ul>
	<li>Уверены ли вы, что имя пользователя и пароль правильные?</li>
	<li>Уверены ли вы, что введено правильное имя сервера баз данных?</li>
	<li>Уверены ли вы, что сервер базы данных запущен и работает?</li>
</ul>
<p>Если вы не уверены в чем-то, вы можете попробовать связаться с провайдером. Если вам нужна помощь, вы всегда можете обратиться на <a href='http://wordpress.org/support/'>Форумы поддержки WordPress</a>.</p>
");
			return;
		}
	...
	function select($db) {
		if (!@mysql_select_db($db, $this->dbh)) {
			$this->ready = false;
			$this->bail("
<h1>Не удается выбрать базу данных</h1>
<p>Мы можем соединиться с сервером (это означает, что имя пользователя и пароль верные) но не можем выбрать базу <code>$db</code>.</p>
<ul>
<li>Вы уверены, что она существует?</li>
<li>Пользователь <code>".DB_USER."</code> имеет права использовать базу данных <code>$db</code>?</li>
<li>На некоторых системах ваши базы данных снабжаются префиксом имени пользователя, например username_wordpress. Не в этом ли проблема?</li>
</ul>
<p>Если вы не знаете как настроить базу данных, вы можете <strong>связаться с вашим провайдером</strong>. Если ничего другое не помогает, вы можете найти помощь на <a href='http://wordpress.org/support/'>форумах поддержки</a>.</p>");
			return;
		}
	}
