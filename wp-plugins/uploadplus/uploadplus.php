...
Description: Security and sanity in file names while uploading. Once activate, please <a href="options-misc.php">define your settings</a>. 
Author: Pixline
Version: 2.7
...
function uploadplus_textdomain() {
	load_plugin_textdomain('uploadplus', FALSE, 'uploadplus');
}

/* init admin panel (now in options/misc) */
function upp_options_init_api() {
	add_settings_section('upp_options_section', __('Upload+ Plugin', 'uploadplus'), 'upp_options_intro', 'misc');

	add_settings_field('uploadplus_cleanlevel', __('Cleaning options', 'uploadplus'), 'upp_options_box_cleanlevel', 'misc', 'upp_options_section');
	add_settings_field('uploadplus_case', __('Case options', 'uploadplus'), 'upp_options_box_case', 'misc', 'upp_options_section');
	add_settings_field('uploadplus_prefix', __('Prefix', 'uploadplus'), 'upp_options_box_prefix', 'misc', 'upp_options_section');
	add_settings_field('uploadplus_utf8toascii', __('Transcription', 'uploadplus'), 'upp_options_box_utf8toascii', 'misc', 'upp_options_section');
...
function upp_options_box_cleanlevel(){
	...
	$styles = array(
		"1" => array('label'=>__('Convert spaces and underscores into dashes', 'uploadplus'), 'demo'=>'wordpress-manual.pdf'), 
		"2" => array('label'=>__('Strip all spaces/dashes/underscores', 'uploadplus'), 'demo'=>'wordpressmanual.pdf'), 
		"3" => array('label'=>__('Convert spaces into underscores (dashes allowed)', 'uploadplus'), 'demo'=>'wordpress-manual.pdf'), 
		);
	foreach($styles as $key=>$info):
		...
		'.$info['label'].' <small>'.__('like in:', 'uploadplus').'</small> <code> '.$info['demo'].' </code></p>
		';
	endforeach;
}
...
function upp_options_box_case(){
	...
	$cases = array(
	"0"	=> __("Leave it whatever it is", 'uploadplus'), 
	"1"	=> __("Make all lowercase", 'uploadplus'), 
	"2"	=> __("Make all UPPERCASE", 'uploadplus')
	);
...
$datebased = array(
	"0" => __('dd (like', 'uploadplus').': '.date('d').$sep.')',
	"1" => __('mmdd (like', 'uploadplus').': '.date('md').$sep.')',
	"2" => __('yymmdd (like', 'uploadplus').': '.date('ymd').$sep.')"',
	"3" => __('yyyymmdd (like', 'uploadplus').': '.date('Ymd').$sep.')',
	"4" => __('yyyymmddhhmm (like', 'uploadplus').': '.date('YmdHi').$sep.')',
	"5" => __('yyyymmddhhmmss (like', 'uploadplus').': '.date('YmdHis').$sep.')',
 	"6" => __('unix timestamp (like', 'uploadplus').': '.date('U').$sep.')',
	);

$otherstyles = array(
	"7" => __('[random (mt-rand)]', 'uploadplus').' '.mt_rand().$sep,
	"8" => __('[random md5(mt-rand)]', 'uploadplus').' '.md5(mt_rand()).$sep,
 	"9" => __('[blog name]', 'uploadplus').' '.str_replace( array(".", " ", "-", "_") ,$sep,strtolower(get_bloginfo('name'))).$sep,
 	"A" => __('[short blog name]', 'uploadplus').' '.str_replace( array(".","_","-"," "),"",strtolower(get_bloginfo('name'))).$sep
	);

$nullval = ($prefix=="") ? 'selected="selected"' : "";
echo '
	<select name="uploadplus_prefix" id="uploadplus_prefix">	
	<option value="" label="'.__('No Prefix', 'uploadplus').'" '.$nullval.'>'.__('No Prefix', 'uploadplus').'</option>
	<optgroup label="'.__('Date Based', 'uploadplus').'">
	';
	$flag = $oflag = "";
	...
	<br/>
	<small>'.__('Prefix will follow the other rules, so if you choose dashes, it will use dashes.', 'uploadplus').'</small>
';
}
...
function upp_options_box_utf8toascii(){
	...
	$options = array(
	"0"	=> __("Don't convert <code>(safe mode)</code>", 'uploadplus'), 
	"1"	=> __("Yes, please, convert utf8 characters into ASCII", 'uploadplus')
	);
	...
	endforeach;
	echo "<br/><small>(".__("Learn more about transcription on <a href='http://en.wikipedia.org/wiki/Transcription_(linguistics)'>Wikipedia</a>", 'uploadplus').").</small>";
}
