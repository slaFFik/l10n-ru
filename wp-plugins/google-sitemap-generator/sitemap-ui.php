		...
		if(!empty($_REQUEST["sm_rebuild"])) { //Pressed Button: Rebuild Sitemap
			...
			if(isset($_GET["sm_do_debug"]) && $_GET["sm_do_debug"]=="true") {
				...
				if(!current_user_can("administrator")) {
					echo '<p>'.__('Please log in as admin', 'sitemap').'</p>';
					return;
				}
				...
				echo '<p>'.__('This is the debug mode of the XML Sitemap Generator. It will show all PHP notices and warnings as well as the internal logs, messages and configuration.', 'sitemap').'</p>';
				echo '<p style="font-weight:bold; color:red; padding:5px; border:1px red solid; text-align:center;">'.__('DO NOT POST THIS INFORMATION ON PUBLIC PAGES LIKE SUPPORT FORUMS AS IT MAY CONTAIN PASSWORDS OR SECRET SERVER INFORMATION!', 'sitemap').'</p>';
				echo "<h3>".__("WordPress and PHP Information", 'sitemap')."</h3>";
				echo '<p>WordPress ' . $GLOBALS['wp_version'] . ' '.__('with', 'sitemap').' ' . ' '.__('DB', 'sitemap').' ' . $GLOBALS['wp_db_version'] . ' '.__('on PHP', 'sitemap').' ' . phpversion() . '</p>';
				echo '<p>'.__('Plugin version', 'sitemap').': ' . $this->sg->GetVersion() . ' (' . $this->sg->_svnVersion . ')';
				echo '<h4>'.__('Environment', 'sitemap').'</h4>';
				echo "<pre>";
				$sc = $_SERVER;
				unset($sc["HTTP_COOKIE"]);
				print_r($sc);
				echo "</pre>";
				echo "<h4>".__("WordPress Config", "sitemap")."</h4>";
				echo "<pre>";
				$opts = array();
				...
				print_r($popts);
				echo "</pre>";
				echo '<h4>'.__('Sitemap Config', 'sitemap').'</h4>';
				echo "<pre>";
				print_r($this->sg->_options);
				echo "</pre>";
				echo '<h3>'.__('Errors, Warnings, Notices', 'sitemap').'</h3>';
				echo '<div>';
				$status = $this->sg->BuildSitemap();
				echo '</div>';
				echo '<h3>'.__('MySQL Queries', 'sitemap').'</h3>';
				if(defined('SAVEQUERIES') && SAVEQUERIES) {
					...
					echo '<h4>'.__('Total Query Time', 'sitemap').'</h4>';
					echo '<pre>' . count($GLOBALS['wpdb']->queries) . ' '.__('queries in', 'sitemap').' ' . round($total,2) . ' '.__('seconds', 'sitemap').'.</pre>';
				} else {
					echo '<p>'.__('Please edit wp-db.inc.php in wp-includes and set SAVEQUERIES to true if you want to see the queries.', 'sitemap').'</p>';
				}
				echo "<h3>".__("Build Process Results", "sitemap")."</h3>";
				echo "<pre>";
				print_r($status);
				echo "</pre>";
				echo '<p>'.__('Done.', 'sitemap').' <a href="' . wp_nonce_url($this->sg->GetBackLink() . "&sm_rebuild=true&sm_do_debug=true",'sitemap') . '">'.__('Rebuild', 'sitemap').'</a> '.__('or', 'sitemap').' <a href="' . $this->sg->GetBackLink() . '">'.__('Return', 'sitemap').'</a></p>';
				echo '<p style="font-weight:bold; color:red; padding:5px; border:1px red solid; text-align:center;">'.__('DO NOT POST THIS INFORMATION ON PUBLIC PAGES LIKE SUPPORT FORUMS AS IT MAY CONTAIN PASSWORDS OR SECRET SERVER INFORMATION!', 'sitemap').'</p>';
				echo '</div>';
				...
				echo '<noscript><a href="' . $redirURL . '">'.__('Click here to continue', 'sitemap').'</a></noscript>';
				exit;
			}
			...
			<form method="post" action="<?php echo $this->sg->GetBackLink() ?>">
				...
				<?php endif; ?>
						...
						<?php $this->HtmlPrintBoxHeader('sm_smres',__('Sitemap Resources:','sitemap'),true); ?>
							<a class="sm_button sm_resGoogle"    href="<?php echo $this->sg->GetRedirectLink('sitemap-gwt'); ?>"><?php _e('Google Webmaster Tools','sitemap'); ?></a>
							...
							<a class="sm_button sm_resBing"     href="<?php echo $this->sg->GetRedirectLink('sitemap-lwt'); ?>"><?php _e('Bing Webmaster Tools','sitemap'); ?></a>
					...
					<?php $this->HtmlPrintBoxHeader('sm_basic_options',__('Basic Options', 'sitemap')); ?>
						...
						<ul>
							<li>
								<label for="sm_b_max_posts"><?php _e('Limit the number of posts in the sitemap:', 'sitemap') ?> <input type="text" name="sm_b_max_posts" id="sm_b_max_posts" style="width:40px;" value="<?php echo ($this->sg->GetOption("b_max_posts")<=0?"":$this->sg->GetOption("b_max_posts")); ?>" /></label> <br/><small>(<?php echo __('Newer posts will be included first', 'sitemap'); ?>)</small>
							</li>
							<li>
							...
							<li>
								...
								<label for="sm_b_style"><?php _e('Include a XSLT stylesheet:', 'sitemap') ?> <input <?php echo ($useDefStyle?'disabled="disabled" ':'') ?> type="text" name="sm_b_style" id="sm_b_style"  value="<?php echo $this->sg->GetOption("b_style"); ?>" /></label><br/><small>
								(<?php _e('Full or relative URL to your .xsl file', 'sitemap') ?>)</small><br/><?php if($this->sg->GetDefaultStyle()): ?><label for="sm_b_style_default"><input <?php echo ($useDefStyle?'checked="checked" ':'') ?> type="checkbox" id="sm_b_style_default" name="sm_b_style_default" onclick="document.getElementById('sm_b_style').disabled = this.checked;" /> <?php _e('Use default', 'sitemap') ?> <?php endif; ?>
							</li>
							<li>
						...
						<div>
							...
							<ul>
								<li>
									...
									<?php _e('Detected Path', 'sitemap') ?>: <?php echo $this->sg->getXmlPath(true); ?><br /><?php _e('Detected URL', 'sitemap') ?>: <a href="<?php echo $this->sg->getXmlUrl(true); ?>"><?php echo $this->sg->getXmlUrl(true); ?></a></small>
								</li>
							</ul>
						</div>
						<div>
							...
							<ul>
								<li>
									<label for="sm_b_filename_manual">
										...
										echo "<br /><small>";
										_e('Example','sitemap');
										echo ": /var/www/htdocs/wordpress/sitemap.xml"; ?></small><br />
								...
								<li>
									<label for="sm_b_fileurl_manual">
										...
										echo "<br /><small>";
										_e('Example','sitemap');
										echo ": http://www.yourdomain.com/sitemap.xml"; ?></small><br />
					...
					<?php $this->HtmlPrintBoxHeader('sm_excludes',__('Excluded items', 'sitemap')); ?>
						...
						<div style="margin:5px 0 13px 40px;">
							<label for="sm_b_exclude"><?php _e('Exclude the following posts or pages:', 'sitemap') ?><br />
							<input name="sm_b_exclude" id="sm_b_exclude" type="text" style="width:400px;" value="<?php echo implode(",",$this->sg->GetOption("b_exclude")); ?>" /></label><br /><small><?php _e('List of IDs, separated by comma', 'sitemap') ?></small><br/>
