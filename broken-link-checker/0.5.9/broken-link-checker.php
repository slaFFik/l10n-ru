...
class ws_broken_link_checker {
   ...
   */
    ...
    function admin_menu(){
        ...
        add_management_page(__('View Broken Links', 'broken-link-checker'), __('Broken Links', 'broken-link-checker'), 'edit_others_posts',
   ...
   */
    ...
    function options_page(){
        ...
        <tr valign="top">
        <th scope="row"><?php _e('Status:', 'broken-link-checker'); ?></th>
        <td>
        ...
        <p><input class="button" type="button" name="recheckbutton" value="<?php _e('Re-check all pages', 'broken-link-checker'); ?>" onclick="location.replace('<?php echo basename($_SERVER['PHP_SELF']); ?>?page=link-checker-settings&amp;recheck=true')" /></p>
        </td>
        </tr>
        ...
        <?php _e('Every', 'broken-link-checker'); ?> <input type="text" name="check_threshold" id="check_threshold"
            value="<?php echo $this->options['check_threshold']; ?>" size='5' maxlength='3'/>
        <?php _e('hours', 'broken-link-checker'); ?>
        <br/>
        <span class="description">
        <?php _e('Links in old posts will be re-checked this often. New posts will be usually checked ASAP.', 'broken-link-checker'); ?>
        </span>
        ...
        <input type="checkbox" name="mark_broken_links" id="mark_broken_links"
            ...
            <label for='mark_broken_links'><?php _e('Apply <em>class="broken_link"</em> to broken links', 'broken-link-checker'); ?></label></label><br/>
        ...
        <tr valign="top">
        <th scope="row"><?php _e('Max. execution time (advanced)', 'broken-link-checker'); ?></th>
        <td>
            ...
            foreach ($links as $link) {
                ...
                <td class='post-title column-title'>
				  ...
				  if ( ('post' == $link['source_type']) || ('custom_field' == $link['source_type']) ){
                  	...
                  	echo "<a class='row-title' href='post.php?action=edit&amp;post=$link[source_id]' title='".__("Edit this post")."'>{$link[post_title]}</a>";
					...
					echo "[".__("An orphaned link! This is a bug.", "broken-link-checker")."]";
                ...
                if ( 'post' == $link['source_type'] ){
					...
					if ( 'link' == $link['instance_type'] ) {	 
						...
						echo "<img src='" . WP_PLUGIN_URL . "/broken-link-checker/images/image.png' class='blc-small-image' alt='".__("Image", "broken-link-checker")."' title='".__("Image", "broken-link-checker")."'> ".__("Image", "broken-link-checker");
					} else {
						echo '[ ??? ]';
					}
					...
					echo "<img src='" . WP_PLUGIN_URL . "/broken-link-checker/images/script_code.png' class='blc-small-image' title='".__("Custom field", "broken-link-checker")."' alt='".__("Custom field", "broken-link-checker")."'> ";
					echo "<code>".$link['link_text']."</code>";
					
				} elseif ( 'blogroll' == $link['source_type'] ){
					//echo $link['link_text'];
					echo "<img src='" . WP_PLUGIN_URL . "/broken-link-checker/images/link.png' class='blc-small-image' title='".__("Bookmark", "broken-link-checker")."' alt='".__("Bookmark", "broken-link-checker")."'> ".__("Bookmark", "broken-link-checker");
				}
				?>
				</td>
                <td class='column-url'>
                ...
                <?php
					...
					$actions['details'] = "<span class='view'><a class='blc-details-button' href='javascript:void(0)' title='".__("Show more info about this link", "broken-link-checker")."'>".__("Details", "broken-link-checker")."</a>";
                  	
					$actions['delete'] = "<span class='delete'><a class='submitdelete blc-unlink-button' title='".__("Remove this link from all posts", "broken-link-checker")."' ".
					...
					if ( $excluded ){
						...
						$actions['exclude'] = "<span class='delete'><a class='submitdelete blc-exclude-button' title='".__("Add this URL to the exclusion list", "broken-link-checker")."' ".
							"id='exclude-button-$rownum' href='javascript:void(0);'>".__("Exclude", "broken-link-checker")."</a>";
					}
					
					$actions['edit'] = "<span class='edit'><a href='javascript:void(0)' class='blc-edit-button' title='".__("Edit link URL", "broken-link-checker")."'>".__("Edit URL", "broken-link-checker")."</a>";
						
					echo '<div class="row-actions">';
					echo implode(' | </span>', $actions);
					
					echo "<span style='display:none' class='blc-cancel-button-container'> ",
						 "| <a href='javascript:void(0)' class='blc-cancel-button' title='".__("Cancel URL editing", "broken-link-checker")."'>".__("Cancel", "broken-link-checker")."</a></span>";
				...
				<td><a href='javascript:void(0);'  
					...
					title='<?php _e('Remove this message and mark the link as valid', 'broken-link-checker'); ?>'><?php _e('Discard', 'broken-link-checker'); ?></a>
				</td>
                <?php } ?>
                </tr>
                <!-- Link details -->
...
jQuery(function($){
	...
	$(".blc-discard-button").click(function () {
		var me = this;
		$(me).html('<?php _e("Wait...", "broken-link-checker"); ?>');
        ...
        $.post(
			...
			function (data, textStatus){
				if (data == 'OK'){
					...
					$(me).html('<?php _e("Discard", "broken-link-checker"); ?>');
					alert(data);
				}
			}
		);
    ...
    $(".blc-edit-button").click(function () {
        ...
        if ( !$(editor).is(':visible') ){
            ...
            editor.focus();
            editor.select();
            edit_button.html('<?php _e("Save URL", "broken-link-checker"); ?>');
        } else {
            editor.hide();
            ...
            if (new_url != orig_url){
                ...
                url_el.html('<?php _e("Saving changes...", "broken-link-checker"); ?>');
					...
					function (data, textStatus){
						...
						if ( data && (typeof(data['error']) != 'undefined') ){
							...
							if ( data.cnt_okay > 0 ){
								...
								if ( data.cnt_error > 0 ){
									var msg = "<?php _e('The link was successfully modifed.', 'broken-link-checker'); ?>";
									msg = msg + "\n" + "<?php _e('However', 'broken-link-checker'); ?>" + ", " + data.cnt_error + " " + "<?php _e('instances couldn&rsquo;t be edited and still point to the old URL.', 'broken-link-checker'); ?>"
									alert(msg);
								} else {
									...
									master.next('.blc-link-details').find('td').html('<center>' + '<?php _e("Loading...", "broken-link-checker"); ?>' + '</center>').load(
										"<?php bloginfo( 'wpurl' ); ?>/wp-admin/admin-ajax.php",
										{
								...
								alert("<?php _e('Something went wrong. The plugin failed to edit', 'broken-link-checker'); ?>" + " " +
									data.cnt_error + " " + "<?php _e('instance(s) of this link.', 'broken-link-checker'); ?>");
			...
			edit_button.html('<?php _e("Edit URL", "broken-link-checker"); ?>');
        }
    });
    ...
    $(".blc-cancel-button").click(function () { 
		...
		master.find('.blc-edit-button').html('<?php _e("Edit URL", "broken-link-checker"); ?>');
    });
        ...
        $.post(
			...
			function (data, textStatus){
				...
				if ( data && ( typeof(data['ok']) != 'undefined') ){
					...
					$(me).html('<?php _e("Unlink", "broken-link-checker"); ?>');
			...
			function (data, textStatus){
				...
				if ( data && ( typeof(data['ok']) != 'undefined' ) ){
					...
					if ( 'broken' == blc_current_filter ){
						...
						$(me).replaceWith('<?php _e("Excluded", "broken-link-checker"); ?>');
					...
					$(me).html('<?php _e("Exclude", "broken-link-checker"); ?>');
					alert(data.error);
				}
			}
		);
	...
	function link_details_row($link){
    	...
    	<ol style='list-style-type: none; width: 50%; float: right;'>
    		<li><strong><?php _e('Log:', 'broken-link-checker'); ?></strong><br/>
    	...
    	<span class='check_date'><?php
    		...
    		if ( $last_check < strtotime('-10 years') ){
				echo __('Never', 'broken-link-checker');
			} else {
    	...
    	<span class='request_duration'><?php 
    		printf(__ngettext('%2.3f second', '%2.3f seconds', $link['request_duration'], 'broken-link-checker'), $link['request_duration']); 
    	?></span></li>
    	...
    	<li><br/><?php _e("This link has failed", "broken-link-checker"); ?> 
    	<span class='check_count'><?php printf(__ngettext('%d time', '%d times', $link['check_count'], 'broken-link-checker'), $link['check_count']) ?></span>.</li>
    	<?php } ?>
		</ol>
		<?php
	}
		...
		while ( $links = $wpdb->get_results($link_q, ARRAY_A) ){
			...
			foreach ($links as $link) {
        		...
        		if ( !$this->is_excluded( $link['url'] ) ) {
					...
					$link_obj->log = __("This link wasn&rsquo;t checked because a matching keyword was found on your exclusion list.", "broken-link-checker");
					$link_obj->save();
				} 
...
*/	
	...
	function status_text( $status ){
		...
		if( $status['broken_links'] > 0 ){
			$text .= sprintf( "<a href='%stools.php?page=view-broken-links' title='".__("View broken links", "broken-link-checker")."'><strong>".__ngettext("Found %d broken link", "Found %d broken links", $status['broken_links'], "broken-link-checker")."</strong></a>",
			  get_option('wpurl'), $status['broken_links']);
		} else {
		...
		if( $status['unchecked_links'] > 0) {
			$text .= sprintf(__ngettext("%d URL in the work queue.", "%d URLs in the work queue.", $status['unchecked_links'], "broken-link-checker"), $status['unchecked_links']);
		} else {
		...
		if ( $status['known_links'] > 0 ){
			$text .= sprintf(__ngettext("Detected %d unique URL", "Detected %d unique URLs", $status['known_links'], "broken-link-checker"), $status['known_links'])." ".sprintf(__ngettext("in %d link", "in %d links", $status['known_instances'], "broken-link-checker"), $status['known_instances']);
			if ($this->options['need_resynch']){
...
*/	
	...
	function ajax_discard(){
		...
		if ( isset($_POST['link_id']) ){
			...
			if ( !$link->valid() ){
				die(__("Oops, I can't find the link", "broken-link-checker")." ".intval($_POST['link_id']) );
			}
	...
	function ajax_edit(){
		...
		if ( isset($_GET['link_id']) && !empty($_GET['new_url']) ){
			...
			if ( !$link->valid() ){
				die( json_encode( array(
					'error' => __("Oops, I can't find the link", "broken-link-checker")." ".intval($_GET['link_id']) 
				 )));
			}
	...
	function ajax_unlink(){
		...
		if ( isset($_POST['link_id']) ){
			...
			if ( !$link->valid() ){
				die( json_encode( array(
					'error' => __("Oops, I can't find the link", "broken-link-checker")." ".intval($_POST['link_id']) 
				 )));
			}
	...
	function ajax_link_details(){
		...
		if ( is_array($link) ){
			...
			die (__("Failed to load link details", "broken-link-checker")." (" . $wpdb->last_error . ")");
		}
	}
	...
	function ajax_exclude_link(){
		...
		if ( isset($_POST['link_id']) ){
			...
			if ( !in_array( $link->url, $this->options['exclusion_list'] ) ){
				...
				$link->log = __("This link wasn&rsquo;t checked because a matching keyword was found on your exclusion list.", "broken-link-checker");
				$link->save();
			}
